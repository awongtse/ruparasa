const state = { loggedIn: false, works: [], products: [], activeFilter: '' };
...
function filteredHomeWorks() {
  if (!state.activeFilter) return state.works;
  return state.works.filter(w => w.type === state.activeFilter);
}
...
function makeCard(w) {
  ...
  card.addEventListener('click', (e) => {
    if (e.target.closest('.like-btn') || e.target.closest('.del-btn')) return;
    openWork(w.id);
  });
  return card;
}
